...
- Upgrade DS→OCR transport to ZeroMQ/IPC (Week 3 target) — OCR PULL implemented; DS PUSH pending
...
Next 3 Actions
1) Add DS sender hook (ZeroMQ PUSH) guarded in `crop_probe.cpp`.
2) Run local smoke for `tools/stream_cli_smoke.sh` (uses `e2e-json-stream`) and capture NDJSON + timing.
